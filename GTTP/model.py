...
class SummarizationModel(object):
	...
	def _add_seq2seq(self):
		...
		with tf.variable_scope('seq2seq'):
			...
			# Add the query encoder.
			que_outputs, fw_st_q, bw_st_q = self._add_query_encoder(emb_que_inputs, self._que_lens)
			self._que_states = que_outputs



			# Our encoder is bidirectional and our decoder is unidirectional so we need to reduce the final encoder hidden state to the right size to be the initial decoder hidden state
			self._dec_in_state = self._reduce_states(fw_st, bw_st) #get this verified
	...
	def run_encoder(self, sess, batch):
		...
		(enc_states, que_states, dec_in_state, global_step) = sess.run([self._enc_states, self._que_states, self._dec_in_state, self.global_step], feed_dict) # run the encoder
	...
	def decode_onestep(self, sess, batch, latest_tokens, enc_states, que_states, dec_init_states, prev_coverage, prev_q_coverage):
		...
		if FLAGS.coverage:
			...
			new_q_coverage = [None for _ in range(beam_size)]
